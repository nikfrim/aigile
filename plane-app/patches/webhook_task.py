...
ISSUE_CREATED_WEBHOOK_DELAY_SECONDS = int(os.environ.get("PLANE_ISSUE_CREATED_WEBHOOK_DELAY_SECONDS", "600"))
...
def webhook_activity(
    ...
    """
    try:
        if event == "issue" and verb == "updated" and ISSUE_CREATED_WEBHOOK_DELAY_SECONDS > 0:
            issue = Issue.objects.only("created_at").get(pk=event_id)
            issue_age_seconds = (timezone.now() - issue.created_at).total_seconds()
            if issue_age_seconds < ISSUE_CREATED_WEBHOOK_DELAY_SECONDS:
                logger.info(
                    "Skipping early issue update webhook for %s because create webhook is delayed",
                    event_id,
                )
                return
...
def model_activity(model_name, model_id, requested_data, current_instance, actor_id, slug, origin=None):
    ...
    if current_instance is None:
        countdown = ISSUE_CREATED_WEBHOOK_DELAY_SECONDS if model_name == "issue" else 0
        webhook_activity.apply_async(
            countdown=countdown,
            kwargs={
                "event": model_name,
                "verb": "created",
